...
// Reference from https://gist.github.com/Chmarusso/045ee79fa9a1fae55928a613044c9067 
library Base64 {
...
contract myNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    public onlyOwner {
        ...
        for (uint i = 0; i < info_to_tree.length; i++) {
            hashes.push(keccak256(abi.encodePacked(info_to_tree[i])));
        }

        uint n = info_to_tree.length;
        uint offset = 0;
        ...
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    ...
                    '"attributes": [{"trait_type": "anumber", "value": ', uint2str(attributes[tokenId].anumber), '},',
                    '{"trait_type": "description", "value": "', attributes[tokenId].description, '"}',
                    ']}'
                )
            ))
        );
